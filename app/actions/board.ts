...
const NewProjectSchema = z.object({
  ...
  dtype: z.string().min(1, "Deliverable type is required."),
...
export async function createProjectAction(input: NewProjectInput) {
  ...
  const [dbMembers, dbDtypeRows, dbMarketRows] = await Promise.all([
    db.member.findMany({ select: { name: true } }),
    db.deliverableType.findMany({ select: { name: true } }),
    db.market.findMany({ select: { name: true } }),
  ]);
  // Designers have no fixed-roster fallback — the roster (Member table) is
  // the only source of valid designer names; see lib/data.ts's comment on why
  // the fixed ROSTER designer names are deliberately not seeded in there.
  const validDesigners = new Set(dbMembers.map((m) => m.name));
  if (!validDesigners.has(data.designer)) throw new Error("Unknown designer.");
  const validDtypes = new Set([...DTYPES, ...dbDtypeRows.map((d) => d.name)]);
  if (!validDtypes.has(data.dtype)) throw new Error("Unknown deliverable type.");
  const validMarkets = new Set([...MARKETS, ...dbMarketRows.map((m) => m.name)]);
  if (!validMarkets.has(data.market)) throw new Error("Unknown market.");
